...
**gpt-5-nano does NOT work with the Chat Completions API** — it returns empty content. You MUST use the **Responses API** instead.

### Use the Responses API (NOT Chat Completions)

```typescript (just as an example, not necessarily the specific language)
...
- `isPro` is available server-side via session and should be able to be passed to create/edit UI components — use server-side gating for enforcement, UI gating for button visibility requires passing `isPro` as a prop or fetching it client-side
