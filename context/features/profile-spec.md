...
- Display user info: email, username, avatar (GitHub or initials), account creation date
- Show usage stats: total items, total collections, breakdown by item type
- Add account actions: change password, delete account with confirmation
- Follow existing codebase patterns for data fetching and components

## Notes

- Avatar logic: Use GitHub avatar from OAuth if available, otherwise generate initials from username/email
- Change password button should only appear for users who signed up with username/email (not GitHub OAuth)
