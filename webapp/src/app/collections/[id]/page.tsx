...
export default async function CollectionItemsPage({
  ...
  try {
    [collections, result] = await Promise.all([
      getDashboardCollections({ accessToken, pageSize: 500 }),
      getItemsByCollection(id, { accessToken, page, pageSize: 5 }),
    ]);
  } catch (error) {
  ...
  const collection = collections.items.find((c) => c.id === id);
        ...
        <main className="min-h-screen bg-background">
          <section className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-12 sm:px-6 lg:px-8">
            <div className="rounded-2xl border border-red-500/20 bg-red-500/5 px-5 py-4">
              <p className="text-sm font-medium text-red-400">Could not load collection. The API may still be starting up.</p>
    ...
    <main className="min-h-screen bg-background">
      <section className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-4">
          <div className={cn("flex size-12 items-center justify-center rounded-2xl border", typeColorClass)}>
